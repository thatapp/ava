...
*   **Service accounts** - These are specific roles with set permissions to perform specific operations with system infrastructure and services. Details of these roles and permissions [presented elsewhere]({{site.data.tenant.apiDocsUri}}/v2#/permissions/get_permissions). This article would not extend on these accounts and permissions.
...
1.  Use HTTP `GET` call to the platform API `/v2/tenants/{TENANT_ID}/roles` endpoint using the TenantAdmin credentials to get the current list of roles and permissions. More about this API call [here]({{site.data.tenant.apiDocsUri}}/v2#/tenants/get_tenants__tenant_id__roles).
...
[group of permissions]({{site.data.tenant.apiDocsUri}}/v2#/permissions/get_permissions)
