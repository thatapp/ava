...
### Update @thatapp.io/jsonata-moment to avoid vulnerability
Updated `@thatapp.io/jsonata-moment` to `1.1.6` to fix a vulnerability found in jsonata `1.8.6` (migrated jsonata to `1.8.7`).

### Update @thatapp.io/component-commons-library to avoid vulnerability
Got rid of `@thatapp.io/component-commons-library` lib as a dependency to avoid circular dependency. The only function that was used from that library has been moved to this library source code.

### Update JSONata to 1.8.7 to resolve vulnerability
Update JSONana version in the `jsonata-moment` library.

### Update @thatapp.io/maester-client to new version to avoid vulnerability
`@thatapp.io/maester-client` updated to `5.0.2`
The new `thatapp-sailor-nodejs` version `2.7.2` [was released](https://www.npmjs.com/package/thatapp-sailor-nodejs).
