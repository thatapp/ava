...
The attribute allows you to enable and disable various automatic Platform emails. Related new API docs can be found [here]({{site.data.tenant.apiDocsUri}}/v2#/tenants/post_tenants) and [here](/white-label/platform-emails/templates).
...
We introduced the `$getPassthrough()` functionality, which allows you to avoid hardcoded `thatapp` mentions while refering the passthrough step. Now you can use `$getPassthrough().step_1.body.value` instead of `thatapp.step_1.body.value`.
