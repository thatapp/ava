...
Enhancing workspace management capabilities with the introduction of a new [API endpoint `GET /workspaces/get_workspaces_all_list`]({{site.data.tenant.apiDocsUri}}/docs/v2#/workspaces/get_workspaces_all_list) designed to deliver a sorted list of available workspaces based on user permissions within the corresponding contract. 
...
Implemented a [search functionality]({{site.data.tenant.apiDocsUri}}/docs/v2#/recipes/get_recipes) enhancement for the `GET v2/recipes` endpoint, allowing users to search for Recipes based on a specific word or phrase contained within the recipe title.
